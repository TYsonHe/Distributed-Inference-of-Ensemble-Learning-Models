...
class ScalingCenter:
    ...
    def _autoScaleBasedOnRequests(self, model_name, min_replica_count, max_replica_count, threshold):
        ...
        print(
            f"{model_name} : Current request count {request_count}, current replica count {replica_count}, target replica count {target_replica_count}")
    ...
    def autoScaleOrignal(self):
        ...
        schedule.every(polling_interval).seconds.do(self._autoScaleBasedOnRequests, 'yolo-darknet', min_replica_count,
                                                    max_replica_count, threshold)
